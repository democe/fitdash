...
// Fitbit REST API helper. No Plasma or Quickshell dependencies — pure QML/XHR.
QtObject {
    id: api
    ...
    function handleHttpError(xhr, context) {
        ...
        if (xhr.status === 0) {
            ...
            api.lastRequestStatus = "Network error at " + time
            api.lastRequestState = "error"
            api.error(api.errorMessage)
            return true
        }
        if (xhr.status === 401) {
            api.lastRequestStatus = "Token expired at " + time + " — refreshing"
        ...
        if (xhr.status === 429) {
            ...
            api.lastRequestStatus = "Rate limited at " + time + " — showing cached data"
            api.lastRequestState = "warn"
            api.error(api.errorMessage)
            return true
        }
        if (xhr.status >= 500) {
            ...
            api.lastRequestStatus = "Server error (HTTP " + xhr.status + ") at " + time
            api.lastRequestState = "error"
            api.error(api.errorMessage)
            return true
        }
        if (xhr.status !== 200) {
            ...
            api.lastRequestStatus = "Error (HTTP " + xhr.status + ") at " + time
            api.lastRequestState = "error"
            api.error(api.errorMessage)
            return true
        }
    ...
    function fetchActivity() {
        ...
        xhr.onreadystatechange = function() {
            ...
            try {
                ...
                if (goals.steps) api.stepsGoal = goals.steps
                api.lastUpdatedTimestamp = Date.now()
                api.lastUpdated          = new Date().toLocaleTimeString()
                api.lastRequestStatus    = "OK — updated at " + api.lastUpdated
                api.lastRequestState     = "ok"
                api.isLoading            = false
                api.dataUpdated()
            } catch(e) {
                api.errorMessage      = "Failed to parse activity data"
                api.lastRequestStatus = "Parse error at " + new Date().toLocaleTimeString()
                api.lastRequestState  = "error"
                api.error(api.errorMessage)
                api.isLoading = false
            }
    ...
    function fetchHeartRate() {
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState !== XMLHttpRequest.DONE) return
            if (xhr.status === 401) return  // handled by activity call
